...
- ブラウザへのファイルダウンロード、公開URLまたは有効期限付き署名URLと`curl`・`wget`・`aria2c`例の表示
- 専用ファイルブラウザで複数フォルダーから対象を選び、URL・`curl`・`wget`・`aria2c`を一括生成（バケット全体検索対応）
- 一括ダウンロードの選択内容をバケット単位の名前付きテンプレートとして保存・再利用
